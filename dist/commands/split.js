...
exports.setupSplitCommand = setupSplitCommand;
const services_1 = require("../db/services");
function setupSplitCommand(bot) {
    // /split 100 Dinner /with Bob Alice
    bot.command("split", async (ctx) => {
        const text = ctx.message.text;
        const parts = text.split(" /with ");
        if (parts.length < 2) {
            return ctx.reply("Usage: /split <amount> <description> /with <friend1> <friend2>...");
        }
        ...
        try {
            const result = await (0, services_1.splitExpenseWithFriends)(ctx.from.id, amount, description, friendNames);
